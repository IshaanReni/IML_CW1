...
#contains functions and class variables outside the DecisionTree scope
    ...
    @classmethod
    def find_split(cls, dataset, history):   # entropy_for_all_columns
        min_entropy = 999999
        ...
        for emitter in range(numcols-1):                #for each column
            ...
            for split_point_index in range(1,len(sorted_data)):         #for each emitter value
                ...
                if sum_entropy < min_col_entropy:               # Checks if the entropy that was just calculated is lower than the lowest for this emitter so far
                    ...
                    min_col_split = greater_split_array[0][0]     # Which split value gave the lowest entropy sum for this emitter
    ...
    @classmethod
    def decision_tree_learning (cls, data, history, depth=1):   #depth 1 by default
        ...
        if len(room_labels) == 1 or depth == Classifier.max_depth:    #if all samples from the same room or max_depth reached (early stopping)
            ...
            attribute, value = Classifier.find_split(data, history)    #find optimal attribute and value to split by for this subset
            history.append([attribute, value])   #add decision to breadcrumb trail, to remember not to choose this again down this branch
    ...
    def fit (cls, dataset, max_depth):
        ...
        history = []  #breadcrumb array to make sure same decision values aren't chosen again.
        tree.root, tree.final_depth = Classifier.decision_tree_learning(Classifier.dataset, history)  #start recursive training process
        return tree
